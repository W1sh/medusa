...
public final class StatusCommandService implements ApplicationCommandService {
    ...
    public StatusCommandService(SlashCommandService slashCommandService) {
    ...
    public Mono<Void> reply(SlashCommandEvent event) {
        return Mono.zip(event.getClient().getGuilds().count(), event.getClient().getUsers().count(), slashCommandService.countAll())
                .flatMap(tuple -> event.reply(spec -> spec.addEmbed(embedCreateSpec -> {
                    ...
                    embedCreateSpec.addField(Messages.ZERO_WIDTH_SPACE, Messages.ZERO_WIDTH_SPACE, true);
